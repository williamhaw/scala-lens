...
object AccountLenses {
  val streetNumberLens: Lens[Street, Int] =
    Lens[Street, Int](get = _.number, set = (a, b) => a.copy(number = b))
  val addressStreetLens: Lens[Address, Street] =
    Lens[Address, Street](get = _.street, set = (a, b) => a.copy(street = b))
  ...
  val userAddressLens: Lens[User, Address] =
    Lens[User, Address](get = _.address, set = (a, b) => a.copy(address = b))
  val accountUserLens: Lens[Account, User] =
    Lens[Account, User](get = _.user, set = (a, b) => a.copy(user = b))
  val accountStreetNumberLens: Lens[Account, Int] =
    compose(accountUserLens, compose(userAddressLens, compose(addressStreetLens, streetNumberLens)))
}
